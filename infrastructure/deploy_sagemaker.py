...
def deploy_model(
    ...
    try:
        # Step 1: Create model.tar.gz containing the model file
        model_tar_path = os.path.join(tmpdir, 'model.tar.gz')
        with tarfile.open(model_tar_path, 'w:gz') as tar:
            # Add model file to the tar with the expected name
        ...
        # Note: SKLearn container already has pandas==1.1.3, numpy, scikit-learn, joblib
        # We only need to install LightGBM and avoid conflicting with pre-installed packages
        # Use a specific LightGBM version that's compatible and stable
        ...
        with open(requirements_path, 'w') as f:
            ...
            # Installing them causes version conflicts
        ...
        # Try Python 3.9 first, fall back to py3 (Python 3.7)
        try:
            container_image = image_uris.retrieve(
                framework='sklearn',
                region=region,
                version='1.0-1',
                py_version='py39',  # Try Python 3.9
                instance_type=instance_type
            )
            print(f"   Using container: {container_image} (Python 3.9)")
            # Use requirements - only LightGBM (container has other packages)
            requirements_path = os.path.join(source_dir, 'requirements.txt')
            with open(requirements_path, 'w') as f:
                f.write("lightgbm==3.3.5\n")  # Use 3.3.5 which has pre-built wheels
                # Don't specify pandas, numpy, scikit-learn, joblib - container already has them
            print(f"   ✅ Updated requirements.txt for Python 3.9 (LightGBM 3.3.5)")
        except Exception as e:
            print(f"   ⚠️ Python 3.9 not available, falling back to Python 3.7")
            print(f"   Error: {e}")
            container_image = image_uris.retrieve(
                framework='sklearn',
                region=region,
                version='1.0-1',
                py_version='py3',  # Python 3.7
                instance_type=instance_type
            )
            print(f"   Using container: {container_image} (Python 3.7)")
            # Ensure requirements.txt only has LightGBM (container has other packages)
            requirements_path = os.path.join(source_dir, 'requirements.txt')
            with open(requirements_path, 'w') as f:
                f.write("lightgbm==3.3.5\n")  # Use 3.3.5 which has pre-built wheels
                # Don't specify pandas, numpy, scikit-learn, joblib - container already has them
            print(f"   ✅ Using Python 3.7 compatible requirements.txt (LightGBM 3.3.5 - has wheels)")
...
def main():
    ...
    parser.add_argument('--endpoint-name', type=str, default='fraud-detection-endpoint',
